...
We see the lowest points of the 99.9% confidence intervals through May are around 108,589 daily pick ups 
...
We can see how devasting the pandemic was for the yellow cabs and how drastic the differnce us between projection and actuality. Even when having 99.9% confidence, our predictions are wrong within a matter of days. 

## For Hire Vehicles
 
Now we turn our focus to the FHV and see how drastic the pandemic was for this service in comparision. As mentioned before we will only focus on from february 2019 forward due to the policy changes within the city. 
...
In the spectral analysis we see some very similar things to the yellow cab analysis with the large spikes in frequency corresponding to weekly cycles. We also see some seasonality its stuggling to identify. The time series plot itself is interesting as there are random spikes and decreases seen at times ~30 and ~200. This suggests it may be difficult to model.

We use a log transform for the inconsistent variance throughough.
```{r}
...
Although there do seem to be some random spikes in the residuals that the model struggles with, it again may be due to thing like holidays. We again follow up with a Ljung-Box test.
...
We again fail to reject the null hypothesis that there is autocorrelation in the residuals. We therefore believe that these predictions will be reasonible and continue with the analysis. 

We continue with the same analysis getting the forecast with a confidence interval at the 99.9% confidence level, but this time for FHV.
...
Interestingly, we see that these COVID-19 drop offs are actually WITHIN the 99.9% confidence intervals for the FHV vehicles! 